...
def seed_from_common_voice(
    ...
    *,
    ...
    for speaker_id in speaker_ids:
        ...
        sf.write(
            str(voicebank_dir / rel_clip), merged, output_sr, subtype="PCM_16"
        )
        ...
        out.append(
            VoiceClip(
                ...
                accent=_normalize_accent(
                    first_row.get("accents") or first_row.get("variant")
                ),
                sample_sentences=sentences[:3],
                license="CC0",
                notes=f"Common Voice ({tsv_path.name}); speaker {speaker_id[:8]}…",
            )
        )
